...
public class StockEndpoint {
	...
	public Response deleteById(@PathParam("id") Long id) {
		Stock entity = em.find(Stock.class, id);
		if (entity == null) {
	...
	@GET
	@Path("/prdid/{prdid:[a-zA-Z_0-9][a-zA-Z_0-9]*}")
	@Produces("application/json")
	public Response findByPrdId(@PathParam("prdid") String prdid) {
		TypedQuery<Stock> findByIdQuery = em
				.createQuery(
						"SELECT DISTINCT s FROM Stock s WHERE s.ProductID = :productId ORDER BY s.id",
						Stock.class);
		findByIdQuery.setParameter("productId", prdid);
		Stock entity;
		try {
			entity = findByIdQuery.getSingleResult();
		} catch (NoResultException nre) {
	...
	@GET
	@Produces("application/json")
	public List<Stock> listAll(@QueryParam("start") Integer startPosition,
			@QueryParam("max") Integer maxResult) {
		TypedQuery<Stock> findAllQuery = em.createQuery(
				"SELECT DISTINCT s FROM Stock s ORDER BY s.id", Stock.class);
		...
		final List<Stock> results = findAllQuery.getResultList();
		return results;
	}
